...
public class LichSuXemPhongController {
    private final LichSuXemPhongRepository repository;
    private final NguoiDungRepository nguoiDungRepository;
    private final PhongTroRepository phongTroRepository;
    private final PhongCardService phongCardService;

    public LichSuXemPhongController(LichSuXemPhongRepository repository, NguoiDungRepository nguoiDungRepository,
    ...
    public ResponseEntity<List<PhongCardDto>> getMine(@PathVariable Integer userId, Authentication auth) {
        ...
        return ResponseEntity.ok(repository.findByNguoiDung_MaNguoiDungOrderByThoiGianXemDesc(userId).stream()
                .map(x -> phongCardService.toCard(x.getPhong(), x.getThoiGianXem())).toList());
    }

    @PostMapping("/nguoi-dung/{userId}/phong/{roomId}")
    public ResponseEntity<PhongCardDto> record(@PathVariable Integer userId, @PathVariable Integer roomId, Authentication auth) {
        checkSelf(userId, auth);
        ...
        LichSuXemPhong saved = repository.save(LichSuXemPhong.builder().nguoiDung(user).phong(room).build());
        return ResponseEntity.ok(phongCardService.toCard(saved.getPhong(), saved.getThoiGianXem()));
    }
